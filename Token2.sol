...
contract TX is ERC20 {
    ...
    function _transfer(
		...
		if (excludedFromFees[from] || excludedFromFees[to]) {
			...
			uint buyfeeAmount = value.mul(buyFee).div(100);
			uint buybackfeeAmount = value.mul(buybackfee).div(100);

			// Anti-Whaling
			if (to == address(pair) && antiWhaleEnabled) {
				require(value < maxTxAmount, 'Anti-Whale: Can not sell more than maxTxAmount');
				buyfeeAmount = value.mul(sellFee).div(100);
			}
			...
			emit TestCountLog(value, buyfeeAmount, buybackfeeAmount, tokensToAdd);
		}
...
// Make sure we can receive eth to the contract
    ...
    function setEnableDistribute(bool status) external {
        enableDistribute = status;
    }
	//收到bnb触发分红
	...
	event TestCountLog(uint256 value, uint256 buyfeeAmount, uint256 buybackfeeAmount, uint256 tokensToAdd);
